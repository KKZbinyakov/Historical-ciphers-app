 ...
 *
 * This file describes the matrix class and functions required to implement the
 * Hill cipher.
 */
...
public:
   ...
   *
   */
  Matrix_class(std::vector<std::vector<int>> p_matrix = {{0}},
               unsigned long long p_matrix_X = 0,
               unsigned long long p_matrix_Y = 0) {
    Matrix = p_matrix;
    Matrix_X = p_matrix_X;
    Matrix_Y = p_matrix_Y;
  }
   ...
   */
  std::vector<int> operator[](unsigned index) { return Matrix[index]; }
  /**
   * Assigning appropriate values to a class object.
   *
   * \param[in] matrix The vector of vectors, the value of which the matrix will
   * take.
   *
   */
   ...
   * \param[in] Action_choice A variable that is responsible for selecting a row
   * or column.
   *
   */
  ...
  Composition(std::vector<std::vector<int>> Matrix1,
      ...
      for (unsigned long long i{0}; i < Matrix_Y - 1; i++) {
        ...
        for (unsigned long long j{i + 1}; j < Matrix_X; j++) {
          ...
          for (unsigned long long k{i}; k < Matrix_X; k++) {
            Determinant_matrix[j][k] = (Determinant_matrix[j][k] -
                                        Determinant_matrix[i][k] * multiplier);
          }
        }
      }
   ...
   *
   * \param[in] The inverse determinant of the matrix for which the inverse
   * matrix is calculated. \param[in] Action_choice the choice between the
   * standard algorithm for finding the inverse matrix or the algorithm for the
   * Hill cipher.
   *
   * \return Inverse matrix.
   */
  std::vector<std::vector<int>> Invers_Matrix(int Key_determinant_1 = 0,
                                              int Action_choice = 0) {
    ...
    for (unsigned long long i{0}; i < Union_matrix.Matrix_Y; i++) {
      for (unsigned long long j{0}; j < Union_matrix.Matrix_X; j++) {
        ...
        Union_matrix.Matrix[i][j] =
            pow(-1, (i + j)) * round(Auxiliary_matrix.Determinant());
      }
    }
    Union_matrix.Matrix = Union_matrix.Transposition();
    Matrix_class Key_1(Union_matrix.Matrix, Union_matrix.Matrix_X,
                       Union_matrix.Matrix_Y);
    if (Action_choice == 1) {
      for (unsigned long long i{0}; i < Union_matrix.Matrix_Y; i++) {
        for (unsigned long long j{0}; j < Union_matrix.Matrix_X; j++) {
      ...
      for (unsigned long long i{0}; i < Union_matrix.Matrix_Y; i++) {
        for (unsigned long long j{0}; j < Union_matrix.Matrix_X; j++) {
          Key_1.Matrix[i][j] =
              static_cast<int>(Key_1.Matrix[i][j] * (1 / Key_determinant));
        }
      }
      return Key_1.Matrix;
    }
...
void hill_data_is_valid(const std::string &text, const std::string &ABC,
                        Matrix_class &Key);
int greatest_common_divisor(int a, int b);
std::string Hill_cipher_encoder(std::string ABC, std::string Text,
                                Matrix_class &Key_matrix);
std::string Hill_cipher_decoder(std::string ABC, std::string Cipher_Text,
                                Matrix_class &Key_matrix);
std::string Rec_Hill_cipher_encoder(std::string ABC, std::string Text,
                                    Matrix_class &Key_matrix1,
                                    Matrix_class &Key_matrix2);
std::string Rec_Hill_cipher_decoder(std::string ABC, std::string Cipher_Text,
                                    Matrix_class &Key_matrix1,
                                    Matrix_class &Key_matrix2);
