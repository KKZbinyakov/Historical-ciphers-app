...
/**
 * A function that checks the correctness of the entered data for the Hill
 * cipher.
 *
 ...
 * \throw Unknown text symbol when there is a symbol in the text that is not in
 * the alphabet. \throw Invalid Key matrix when the determinant of the matrix
 * does not satisfy the conditions of the algorithm.
 *
 */
void hill_data_is_valid(const std::string &text, const std::string &ABC,
  ...
  if (greatest_common_divisor(m, Key_determinant) != 1 ||
      Key_determinant == 0) {
    throw input_data_invalid("Invalid Key matrix");
  }
}
...
std::string Hill_cipher_encoder(std::string ABC, std::string Text,
  ...
  try {
    ...
    Matrix_class Size_matrix(size_matrix, size_matrix[0].size(),
                             size_matrix.size());
    while (i <= Lenght_of_text) {
      ...
      Size_matrix.Matrix =
          Key_matrix.Composition(Key_matrix.Matrix, Size_matrix.Matrix);
      for (int j{0}; j < Key_size; j++) {
...
std::string Hill_cipher_decoder(std::string ABC, std::string Cipher_Text,
  ...
  try {
    ...
    Matrix_class Key_1(Key_matrix.Invers_Matrix(Key_determinant_1, 1),
                       Key_matrix.Matrix_X, Key_matrix.Matrix_Y);
    std::vector<int> v1{0};
    std::vector<std::vector<int>> size_matrix;
    for (int i{0}; i < Key_size; i++) {
      auto iter = size_matrix.cbegin();
...
std::string Rec_Hill_cipher_encoder(std::string ABC, std::string Text,
  ...
  try {
    ...
    for (int i{0}; i < Key_count - 2; i++) {
      Matrix_class Short_time_key(
          Key_set[i].Composition(Key_set[i].Matrix, Key_set[i + 1].Matrix),
          Key_set[i + 1].Matrix_X, Key_set[i + 1].Matrix_Y);
  ...
  try {
    ...
    for (int i{0}; i < Key_count - 2; i++) {
      Matrix_class Short_time_key(
          ...
          Key_set[i + 1].Matrix_X, Key_set[i + 1].Matrix_Y);
      Part_of_Text =
          Cipher_Text.substr(Key_size + (i + 1) * Key_size, Key_size);
