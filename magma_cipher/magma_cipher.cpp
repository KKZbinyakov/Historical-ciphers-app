...
private:
    ...
    std::vector<uint8_t> feedbackGammaBuilding(const Block& synchroLink, const std::vector<uint8_t>& data) {
        ...
        for (size_t i = 0; i < dataSize - 1; i += 8) {
            Block block = { static_cast<uint8_t>(N2 >> 24), static_cast<uint8_t>(N2 >> 16), static_cast<uint8_t>(N2 >> 8), static_cast<uint8_t>(N2),
                                 static_cast<uint8_t>(N1 >> 24), static_cast<uint8_t>(N1 >> 16), static_cast<uint8_t>(N1 >> 8), static_cast<uint8_t>(N1) };
            Block gammaBlock = encryptBlock(block);
            for (int j = 0; j < 8; ++j) {
                gamma[i + j] = gammaBlock[j];
            }
            ...
            for (size_t i = 0; i < BLOCK_SIZE; ++i) {
                gammaBlock[i] = data[i] ^ gammaBlock[i];
            }
...
std::vector<uint8_t> magmaEncrypt(const std::vector<uint8_t>& data, const Magma::Key& key) {
    Magma magma(key);
    return magma.encrypt(data);
}

std::vector<uint8_t> magmaDecrypt(const std::vector<uint8_t>& data, const Magma::Key& key) {
    Magma magma(key);
    return magma.decrypt(data);
}

std::vector<uint8_t> magmaGammaEncrypt(const std::vector<uint8_t>& data, const Magma::Key& key, const std::array<uint8_t, 8>& synchroLink) {
    Magma magma(key);
    return magma.gammaAlgorithm(data, synchroLink);
}

std::vector<uint8_t> magmaGammaDecrypt(const std::vector<uint8_t>& data, const Magma::Key& key, const std::array<uint8_t, 8>& synchroLink) {
    Magma magma(key);
    return magma.gammaAlgorithm(data, synchroLink);
}

std::vector<uint8_t> magmaFeedbackGammaEncrypt(const std::vector<uint8_t>& data, const Magma::Key& key, const std::array<uint8_t, 8>& synchroLink) {
    Magma magma(key);
    return magma.feedbackGammaAlgorithm(data, synchroLink);
}

std::vector<uint8_t> magmaFeedbackGammaDecrypt(const std::vector<uint8_t>& data, const Magma::Key& key, const std::array<uint8_t, 8>& synchroLink) {
    Magma magma(key);
    return magma.feedbackGammaAlgorithm(data, synchroLink);
}
...
int main() {
    ...
    std::vector<uint8_t> ciphertext = magmaEncrypt(plaintext, key);
    std::vector<uint8_t> decryptedText = magmaDecrypt(ciphertext, key);
    ...
    Magma magma;
    magma.setKey(key);
    auto retrievedKey = magma.getKey();

    std::cout << "Retrieved key: ";
    for (auto byte : retrievedKey) std::cout << std::hex << (int)byte << " ";
    std::cout << std::endl;

    std::array<uint8_t, 8> synchroLink = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF};
    ciphertext = magmaGammaEncrypt(plaintext, key, synchroLink);
    decryptedText = magmaGammaDecrypt(ciphertext, key, synchroLink);
    ...
    ciphertext = magmaFeedbackGammaEncrypt(plaintext, key, synchroLink);
    decryptedText = magmaFeedbackGammaDecrypt(ciphertext, key, synchroLink);
