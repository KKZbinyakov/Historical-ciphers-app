 ...
 *
 * In that file we describe the functions of 3 types of vigenere cipherings and
 * decipherings, namely short key gamma, self key from opentext and self key
 * from ciphertext, and the validation function for them.
 */

/**
 * The validation function for Vigenere ciphers.
 *
 * \param[in] text The text we want to encrypt or decrypt with some of Vigenere
 * ciphers. \param[in] key The key for Vigenere ciphering. \param[in] alphabet
 * The alphabet of Vigenere ciphering.
 *
 * \throw input_data_invalid with the suitable message in case of wrong
 * alphabet, wrong key or wrong text.
 */
...
/**
 * The function of Vigenere ciphering with gamma formed with repeating short
 * key.
 *
 * \param[in] text The text we want to encrypt or decrypt with some of Vigenere
 * ciphers. \param[in] key The key for Vigenere ciphering. \param[in] alphabet
...
std::string repeat_key_ciphering(const std::string &text,
  ...
  try {
    ...
    for (int i = 0; i < text.size(); i++) {
      int point = std::find(alphabet.begin(), alphabet.end(), text[i]) -
                  alphabet.begin();
      int gam =
          std::find(alphabet.begin(), alphabet.end(), key[i % key.size()]) -
          alphabet.begin();
...
std::string repeat_key_deciphering(const std::string &ciphertext,
  ...
  try {
    ...
    for (int i = 0; i < ciphertext.size(); i++) {
      int point = std::find(alphabet.begin(), alphabet.end(), ciphertext[i]) -
                  alphabet.begin();
      int gam =
          std::find(alphabet.begin(), alphabet.end(), key[i % (key.size())]) -
          alphabet.begin();
...
/**
 * The function of Vigenere ciphering with gamma formed with selfkey from
 * opentext.
 *
 * \param[in] text The text we want to encrypt or decrypt with some of Vigenere
 * ciphers. \param[in] key The key for Vigenere ciphering. \param[in] alphabet
...
std::string self_open_key_ciphering(const std::string &text,
  ...
  try {
    ...
    for (int i = 0; i < text.size(); i++) {
      int first_point = std::find(alphabet.begin(), alphabet.end(), text[i]) -
                        alphabet.begin();
      int sec_point = std::find(alphabet.begin(), alphabet.end(), gam[i]) -
                      alphabet.begin();
...
/**
 * The function of Vigenere deciphering with gamma formed with selfkey from with
 * opentext.
 *
 * \param[in] text The text we want to encrypt or decrypt with some of Vigenere
 * ciphers. \param[in] key The key for Vigenere ciphering. \param[in] alphabet
...
std::string self_open_key_deciphering(const std::string &ciphertext,
  ...
  try {
    ...
    for (int i = 0; i < ciphertext.size(); i++) {
      int first_point =
          std::find(alphabet.begin(), alphabet.end(), ciphertext[i]) -
          alphabet.begin();
      int sec_point = std::find(alphabet.begin(), alphabet.end(), gam[i]) -
                      alphabet.begin();
      decipher += alphabet[((first_point - sec_point) % len_alph + len_alph) %
                           len_alph];
      gam += alphabet[((first_point - sec_point) % len_alph + len_alph) %
                      len_alph];
    }
    return decipher;
  } catch (input_data_invalid error) {
...
/**
 * The function of Vigenere ciphering with gamma formed with selfkey from with
 * ciphertext.
 *
 * \param[in] text The text we want to encrypt or decrypt with some of Vigenere
 * ciphers. \param[in] key The key for Vigenere ciphering. \param[in] alphabet
...
/**
 * The function of Vigenere deciphering with gamma formed with selfkey from the
 * ciphertext.
 *
 * \param[in] text The text we want to encrypt or decrypt with some of Vigenere
 * ciphers. \param[in] key The key for Vigenere ciphering. \param[in] alphabet
