...
MainWindow::MainWindow(QWidget *parent) : QWidget(parent) {
  ...
  layout->addWidget(inputLabel, 0, 1, 1, 1);
...
/**
 *
 * Function clearing the window when a new cipher type is selected in the
 * combobox
 *
...
/**
 *
 * Function changing the layout of the window depending on which cipher was
 * selected and filling some of the input lines with default values
 *
 */
...
/**
 *
 * Function showing the matrix to be filled in if the Hill ciphers type was
 * selected
 *
  ...
  try {
    ...
    if (comboBox->currentText().toStdString() == "Affine ciphers") {
      if (lastClickedButton->text().toStdString() == "Simple replacement") {
        ...
        replacement_cipher_data_is_valid(inputText.toStdString(), key,
                                         alphabet);
        outputLine->setText(
      ...
      if (lastClickedButton->text().toStdString() == "Recurrent affine") {
        ...
        if (!alpha_1.size() || !alpha_2.size() || !beta_1.size() ||
            !beta_2.size()) {
          throw input_data_invalid("Please enter alpha or beta");
        }
        std::vector<int> key{alpha_1.toInt(), alpha_2.toInt(), beta_1.toInt(),
                             beta_2.toInt()};
        affine_reccurent_cipher_data_is_valid(inputText.toStdString(), key,
                                              alphabet);
    ...
    if (comboBox->currentText().toStdString() == "Magma ciphers") {
      ...
      for (int i{0}; i < str_data.size(); i++) {
        if ((i % 3 == 2 && str_data[i] != ' ') ||
            (i % 3 != 2 && std::count(possible.begin(), possible.end(),
                                      str_data.at(i)) == 0)) {
          flag1 = true;
        }
      }
      if (flag1 || str_data.size() != 47) {
        throw input_data_invalid("Data must be in hex, contain 16 blocks, each "
                                 "block separated by 1 space");
      }
      ...
      if (flag2 || str_key.size() != 95) {
        throw input_data_invalid("Key must be in hex, contain 32 blocks, each "
                                 "block separated by 1 space");
      }
      if (lastClickedButton->text().toStdString() == "Magma cipher") {
        std::string str_data = magma_lines[0]->text().toStdString();
        std::string str_key = magma_lines[1]->text().toStdString();
        std::vector<uint8_t> data = hexStringToVector(str_data);
        std::vector<uint8_t> key = hexStringToVector(str_key);
      ...
      if (lastClickedButton->text().toStdString() == "Gamma") {
        ...
        for (int i{0}; i < str_synchrolink.size(); i++) {
          if ((i % 3 == 2 && str_synchrolink[i] != ' ') ||
              (i % 3 != 2 && std::count(possible.begin(), possible.end(),
                                        str_synchrolink.at(i)) == 0)) {
            flag = true;
          }
        }
        if (flag || str_synchrolink.size() != 23) {
          throw input_data_invalid("Synchrolink must be in hex, contain 8 "
                                   "blocks, each block separated by 1 space");
        }
        std::vector<uint8_t> data = hexStringToVector(str_data);
        std::vector<uint8_t> key = hexStringToVector(str_key);
        std::vector<uint8_t> synchrolink = hexStringToVector(str_synchrolink);
  ...
  try {
    ...
    if (comboBox->currentText().toStdString() == "Vigenere ciphers") {
      ...
      if (lastClickedButton->text().toStdString() == "Self cipher key") {
        std::string key{vigenere_Line->text().toStdString()};
        vigenere_data_is_valid(inputText.toStdString(), key, alphabet);
        outputLine->setText(QString::fromStdString(self_cipher_key_deciphering(
            inputText.toStdString(), key, alphabet)));
      }
    }
